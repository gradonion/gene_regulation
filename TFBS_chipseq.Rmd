...
```{r echo=FALSE}
TF = "STAT1"
filename = paste0('~/Downloads/Research/LCL/',TF,'_all_genes_R2_noPC.txt')
 ...
 *  Further pick the top 10% peaks based on their ChIP-se enrichment signal values, and link the target genes similarly trhough Hi-C (also corrected for the top 2 PCs):
...
filename = paste0('~/Downloads/Research/LCL/5kb_promoter_chipseq/R2_target_prom_genes_',TF,'.txt')
rsq_target = read.table(filename, sep = "\t", header = FALSE, na.strings = 'NA', stringsAsFactors = FALSE)

filename = paste0('~/Downloads/Research/LCL/5kb_promoter_chipseq/R2_nontarget_prom_genes_',TF,'.txt')
